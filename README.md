...
```
ghcr.io/ivenos/compwire:latest
```
...
1. Generate keypairs for each node:
   ...
   ```sh
   docker run --rm ghcr.io/ivenos/compwire genkey
   # PRIVATE_KEY=<base64>
   # PUBLIC_KEY=<base64>
   ```
...
|---|:---:|---|---|
| `WG_ROLE` | ✓ | — | Must be `server` |
| `WG_PRIVATE_KEY` | ✓¹ | — | WireGuard private key (base64) |
| `WG_PRIVATE_KEY_FILE` | ✓¹ | — | Path to file containing the private key |
| `WG_PEER_<ID>_PUBKEY` | ✓ | — | Peer public key |
| `WG_PEER_<ID>_ALLOWED_IPS` | ✓ | — | Allowed IP ranges for this peer (comma-separated CIDRs) |
...
| Variable | Required | Default | Description |
|---|:---:|---|---|
| `WG_ROLE` | ✓ | — | Must be `client` |
| `WG_PRIVATE_KEY` | ✓¹ | — | WireGuard private key (base64) |
| `WG_PRIVATE_KEY_FILE` | ✓¹ | — | Path to file containing the private key |
| `WG_SERVER_PUBKEY` | ✓ | — | Server public key |
| `WG_SERVER_ENDPOINT` | ✓ | — | Server address in `host:port` format |
...
1. Generate a keypair for the phone:
   ```sh
   docker run --rm ghcr.io/ivenos/compwire:latest genkey
   # PRIVATE_KEY=<phone-private-key>
...
3. Display the QR code — pass the same env vars you would use for a client container:
   ...
   docker run --rm \
     ...
     ghcr.io/ivenos/compwire:latest showqr
   ```
